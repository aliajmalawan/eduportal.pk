...
(function () {
  ...
  // Resolved from this page's own origin so it works under /eduportal/ locally
  // and at the domain root in production, with no build-time configuration.
  var LOCAL_MIRROR_URL = new URL('api/save-demo-lead.php', document.baseURI).href;
  ...
  /**
   * Mirror the lead into this site's own database.
   *
   * sendLeadToCrm() above uses mode:'no-cors', so its response is opaque and
   * always resolves — the front end cannot tell a saved lead from a dropped
   * one. Without a local copy, a CRM outage loses the enquiry while the
   * visitor is still shown a success message.
   *
   * Deliberately additive: this never blocks or replaces the CRM request, and
   * a failure here is swallowed so it can never turn a delivered lead into a
   * visible error.
   */
  function mirrorLeadLocally(payload, sourcePage) {
    try {
      var body = JSON.stringify({
        instituteName: payload.school || '',
        studentCount: payload.strength || '',
        fullName: payload.name || '',
        designation: payload.role || '',
        whatsapp_full: payload.contact || '',
        source_page: sourcePage || location.pathname,
      });
      return fetch(LOCAL_MIRROR_URL, {
        method: 'POST',
  ...
  window.handleLeadFormSubmit = function (form, callbacks) {
    ...
    // Fired before the CRM call so the local copy is attempted even if the
    // page is closed the instant the CRM request goes out (keepalive).
    mirrorLeadLocally(result.payload, form.dataset.sourcePage);
